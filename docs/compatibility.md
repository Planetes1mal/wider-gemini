...
The shared Tests workflow keeps `windows-latest` and now selects the image's preinstalled Google Chrome instead of downloading Chrome for Testing. It resolves the registered application path used by the [official runner browser checks](https://github.com/actions/runner-images/blob/main/images/windows/scripts/tests/Browsers.Tests.ps1) and logs the actual version. The complete unit/packaging suite, all synthetic layout scenarios, the 30-second timeout, and browser security flags remain unchanged. Failed browser launches or missing reports now include bounded stdout/stderr diagnostics. No extension runtime code changed; release ZIP creation remains on Ubuntu.

**CI acceptance — September 24, 2026:** the [preinstalled-Chrome Tests run](https://github.com/Planetes1mal/wider-gemini/actions/runs/35953787927) completed successfully at commit `379084e5c5f28253a4d7323e59728eaee11fcbb6`, using Google Chrome `152.0.7977.83` at `C:\Program Files\Google\Chrome\Application\chrome.exe`. All unit checks, all 15 packaging checks using `powershell.exe`, and all 73 synthetic layout scenarios passed. This establishes hosted CI acceptance of that source, not live-Gemini/PWA/macOS verification or successful release publication.
